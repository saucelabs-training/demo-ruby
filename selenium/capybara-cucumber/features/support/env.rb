...
Before do |scenario|
  opts = platform.merge(name: "#{scenario.feature.name} - #{scenario.name}",
                        build: build_name)
  sauce_options = SimpleSauce::Options.new(opts)

  @session = SimpleSauce::CapybaraSession.new(sauce_options)
  @session.start
end

After do |scenario|
  @session.stop(scenario.passed?)
end
