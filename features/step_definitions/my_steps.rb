...
When(/^I add (.*) items$/) do |number|
  number.to_i.times { @browser.button(class: 'add-to-cart-button').click }
end
...
And(/^I have (\d+) items in the Cart$/) do |number|
  number.times { @browser.button(class: 'add-to-cart-button').click }
end

When(/^I remove (\d+) item$/) do |number|
  number.times { @browser.button(class: 'remove-from-cart-button').click }
end
